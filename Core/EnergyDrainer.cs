...
namespace TWitchery;
using Liquids;
class EnergyDrainer<T> where T : ModTileEntity {
	// private Tile _tile;
    ...
    public float DrainAltar(float amount,Player ply, int i, int j) {
        ...
        foreach (var step in new Func<bool>[] { () => DrainPlayer(ply), DrainBiome, DrainFlora, DrainBlocks, DrainLivingForms }) ;
    ...
    public float DrainCauldron(float amount, Player ply, LiquidInventory liquids) {
		...
		foreach (var step in new Func<bool>[] { () => DrainPlayer(ply), () => DrainLiquid(liquids) })
			if (_yetToDrain > 0)
				step();
			else
				break;
    ...
    private bool DrainPlayer(Player ply) {
        var couldntTake = ply.GetModPlayer<TWitcheryPlayer>().TakeMana((int)_yetToDrain, useDeplition: false);
        return couldntTake <= 0;
    }

    private bool DrainLiquid(LiquidInventory liquids) {
        if (_yetToDrain == 0)
            return true;
        ...
        return _yetToDrain > 0;
    }

	private bool DrainFlora() {
		int radius = 10;
    ...
    private bool DrainLivingForms()
    {
        ...
        for (int k = 0; k < Main.maxNPCs; k++)
        {
            ...
            {            
                float sqrDistanceToTarget = Vector2.DistanceSquared(
                    new Vector2(target.Center.ToTileCoordinates().X, target.Center.ToTileCoordinates().Y), pos);
